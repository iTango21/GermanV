...
url = 'https://www.chrono24.com/omega/omega-seamaster-aqua-terra-beijing-2008-limited-edition--id29264730.htm'
...
def download_img(image_url, directory_name, zoom):
    # Получаем имя файла из URL
    file_name = os.path.basename(image_url)
    # print(file_name)

    if zoom:
        # Проверяем, существует ли "dir2" в "dir1" и создаем, если не существует
        dir2_path = os.path.join(directory_name, "_zoom_")
    ...
    if os.path.isfile(file_path):
        print(f"Image {file_name} already exists in directory {directory_name}")
    else:
        # print(f"Файл {file_name} не существует в директории {directory_name}")
        ...
        if resp.status_code == 200:
            ...
            print(f"Failed to download image {file_name}. Error status code: {response.status_code}")
